...
public class UserServiceImpl implements IUserService {
	...
	@Override
	public List<UserVo> getAllUsersWithOutSuperAdmin() {
		List<UserVo> users = getAllUsers();
		List<UserVo> usersWithOutSuperAdmin = new ArrayList<>();

		for (UserVo userVo : users) {
			if (!haveSuperAdminRole(userVo.getRoles())){
				usersWithOutSuperAdmin.add(userVo);
			}
		}
		return usersWithOutSuperAdmin;
	}

	private boolean haveSuperAdminRole(List<RoleVo> roles) {
		boolean exist=false;
	...
	@Override
	public List<RoleVo> getAllRolesOtherThenSuperAdmin() {
		return RoleConverter.toVoList(roleRepository.getAllRolesOtherThenSuperAdmin("SUPER_ADMIN"));
	}
}